...
namespace ListasSarlaft.UserControls
{
    public partial class ConsultarUsuario : System.Web.UI.UserControl
    {
        ...
        private DataTable InfoGrid
        {
            get
            {
                infGrid = (DataTable)ViewState["infGrid2"];
                return infGrid;
            }
            set
            {
                infGrid = value;
                ViewState["infGrid2"] = infGrid;
            }
        }
        ...
        private void cargarInfoGrid()
        {
            ...
            if (lstUsuarios != null)
            {
                ...
                GridView1.DataBind(); //gridview a listar para exportar a excel-- yoendyca
            }
            else
            {
                ...
                GridView1.DataBind();
                /*Prueba*/
                //exportExcel(InfoGrid, Response, "ConsultaUsuario");
        ...
        {
            DataTable grid = new DataTable("Table1");
            ...
            grid.Columns.Add("FechaUltActualPassword", typeof(string));
            ...
            foreach (clsDTOUsuarios objUsuarios in lstUsuarios)
            {
                InfoGrid.Rows.Add(new Object[] {
                    objUsuarios.intId.ToString().Trim(),
                            ...
                            objUsuarios.intBloqueado.ToString().Trim(),
                            objUsuarios.strContrasenaEncriptada.ToString().Trim(),
                            objUsuarios.dtFechaUltActualPassword.ToString().Trim(),
                            objUsuarios.FechaCreacion.ToString().Trim()
                    });
            }
        }
        ...
        public void exportExcel(DataTable dt, HttpResponse Response, string filename)
        {
            ...
            {
                Mensaje("Error al exportar el reporte de usuarios: Error  ->" + ex.Message);
            }
            finally
            {
        ...
        protected void Exportar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                DataTable dt = new DataTable();
                dt = InfoGrid;
                dt.Columns.Remove("IdUsuario");
                dt.Columns.Remove("Contrasena");
                dt.Columns.Remove("IdRol");
                dt.Columns.Remove("IdJerarquia");

                dt.Columns[0].ColumnName = "Rol";
                dt.Columns[1].ColumnName = "Jerarquía Organizacional";
                dt.Columns[2].ColumnName = "Número Documento";
                dt.Columns[6].ColumnName = "Estado";
                dt.Columns[7].ColumnName = "Fecha Último Cambio";
                dt.Columns[8].ColumnName = "Fecha Creación";

                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    var valor = dt.Rows[i]["Estado"].ToString();
                    if (valor == "0")
                    {
