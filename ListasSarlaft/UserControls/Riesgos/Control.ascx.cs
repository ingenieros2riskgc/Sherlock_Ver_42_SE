...
namespace ListasSarlaft.UserControls.Riesgos
{
    public partial class Control : System.Web.UI.UserControl
    {
        ...
        private int IndiceSeleccionado
        {
            get
            {
                indiceSeleccionado = (int)ViewState["indiceSeleccionado"];
                return indiceSeleccionado;
            }
            set
            {
                indiceSeleccionado = value;
                ViewState["indiceSeleccionado"] = indiceSeleccionado;
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    scrtManager.RegisterPostBackControl(TreeView1);
                    scrtManager.RegisterPostBackControl(TreeViewJerarquiaOrg);
                    ...
                    {
                        omb.ShowMessage("Error al cargar controles: " + ex.Message.ToString(), 3, "Error");
                        throw;
                    }
                }
            }
        ...
        //        for (int i = 0; i < dtInfo.Rows.Count; i++)
        //        {
        //            cbEstado.Items.Insert(i + 1, new ListItem(dtInfo.Rows[i]["NombreMitiga"].ToString().Trim(), dtInfo.Rows[i]["IdMitiga"].ToString()));
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        ...
        private void loadInfoControles()
        {
            ...
            if (dtInfo.Rows.Count > 0)
            {
                ...
                // botones activar/inactivar controles Yoendy ca
                for (int rowIndex = 0; rowIndex < GridView1.Rows.Count; rowIndex++)
                {
                    string estado = dtInfo.Rows[rowIndex]["Estado"].ToString().Trim();
        ...
        {
            IndiceSeleccionado = (Convert.ToInt16(GridView1.PageSize) * PagIndexInfoGridControles) + Convert.ToInt16(e.CommandArgument);
            ...
            switch (e.CommandName)
            {
                ...
                case "Borrar":
                    borrarControl();
                    break;
                case "Estado": //REQ-03 -yoendy ca

                    if (RowGridControles >= 10)
                    {
                        RowGridControles = Convert.ToInt16(e.CommandArgument);
                    }
                    ...
                    if (lbl.Text == "Activo")
                    {
                        lblMsgBox1.Text = "¿Está seguro que desea inactivar el control?";
                    }
                    else
                    {
                        lblMsgBox1.Text = "¿Está seguro que desea activar el  control?";
                    }
                    mpeMsgBox1.Show();
                    break;
            }
        ...
        protected void GridView1_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            GridView1.DataSource = InfoGridControles;
            GridView1.DataBind();
            int paginaGrid = (Convert.ToInt16(GridView1.PageSize) * PagIndexInfoGridControles);
        ...
        //REQ-03 -Yoendy Ca
        private string cambiaEstado(string nuevoEstado)
        {           //aqui            
            string transaccion = string.Empty;
            string CodRelacion = string.Empty;
            string codigoControl = InfoGridControles.Rows[IndiceSeleccionado]["CodigoControl"].ToString().Trim();
            int evento = 3;
            int estadoActual = 0;
            string result = cControl.cambiaEstado(codigoControl, evento, estadoActual);
            //verifico primero si tiene relación
            if (Convert.ToInt32(result) > 0)
            {
                for (int i = 0; i < cControl.infoRelacion.Rows.Count; i++)
                {
                    CodRelacion += "[" + cControl.infoRelacion.Rows[i]["Codigo"].ToString() + "] ";
                }

                if (cControl.infoRelacion.Rows.Count > 1)
                {
                    omb.ShowMessage("Error: " + "No es posible inactivar el control porque tiene los siguientes riesgos asociados: <br /><br /> <B>" + CodRelacion + " </B> </B> <br /><br /> Se recomienda quitar las relaciones de los riesgos mencionados para proceder a inactivarlos.", 1, "Atención");
                }
                else if (cControl.infoRelacion.Rows.Count == 1)
                {
                    omb.ShowMessage("Error: " + "No es posible inactivar el control porque tiene el siguiente riesgo asociado: <br /><br /> <B>" + CodRelacion + " </B> <br /><br /> Se recomienda quitar la relación del riesgo mencionado para proceder a inactivar. ", 1, "Atención");
                }
            }
            else if (nuevoEstado == "0")
            {
                evento = 1;
                estadoActual = 1;
                //No existe relación ,procede a Inactivar
                result = cControl.cambiaEstado(codigoControl, evento, estadoActual);
                Mensaje("Se inactivó el control satisfactoriamente. ");
                transaccion = "OK";
            ...
            else if (nuevoEstado == "1")
            {
                evento = 1;
                estadoActual = 0;
                //No existe relación ,procede a Activar
                result = cControl.cambiaEstado(codigoControl, evento, estadoActual);
                Mensaje("Se activó el control satisfactoriamente. ");
                transaccion = "OK";
        ...
        private void agregarControl()
        {
            ...
            cControl.agregarControl(Sanitizer.GetSafeHtmlFragment(TextBox1.Text.Trim()), Sanitizer.GetSafeHtmlFragment(TextBox2.Text.Trim()), Sanitizer.GetSafeHtmlFragment(TextBox3.Text.Trim()), Sanitizer.GetSafeHtmlFragment(TextBox4.Text.Trim()),
        ...
        private void resetValuesCamposDetalleEvaluacion()
        {
            ...
            if (TreeView3.SelectedNode != null)
                TreeView3.SelectedNode.Selected = false;
            if (TreeViewJerarquiaOrg.SelectedNode != null)
                TreeViewJerarquiaOrg.SelectedNode.Selected = false;
            if (TreeViewTablaParam.SelectedNode != null)
                TreeViewTablaParam.SelectedNode.Selected = false;
        }
        #endregion Resets
        ...
        protected void IBprocess_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                Session["NombreControl"] = NombreControl;
                ...
                Iframe.Attributes["src"] = "../../../UserControls/Riesgos/Iframe/IFcategorias.aspx?ResponsableEjecucion=" + ResponsableEjecucion + "&IdResponsablesEjecucion=" + IdResponsablesEjecucion +
        ...
        private void mtdShowCalificacion()
        {
            ...
            string DescripcionControl = Sanitizer.GetSafeHtmlFragment(TextBox3.Text);
            Session["DescripcionControl"] = DescripcionControl;
            //string ObjetivoControl = Sanitizer.GetSafeHtmlFragment();
            Session["ObjetivoControl"] = TextBox4.Text;// ObjetivoControl;
            string ResponsableEjecucion = Sanitizer.GetSafeHtmlFragment(TxbResponsableEjecución.Text);
            string IdResponsablesEjecucion = Sanitizer.GetSafeHtmlFragment(LblResponsableEjecucion.Text);
            string IdResponsableCalificacion = Sanitizer.GetSafeHtmlFragment(lblIdDependencia1.Text);
            string Periodicidad = DropDownList6.SelectedValue;
            ...
            if (NombreControl.Length > 200)
            {
               NombreControl = NombreControl.Substring(0 , 200);
            }
            if (DescripcionControl.Length > 200)
            {
                DescripcionControl = DescripcionControl.Substring(0 , 200);
            }
            ...
            /*"&NombreControl=" + NombreControl  +
                "&ObjetivoControl=" + ObjetivoControl*/
            Iframe.Attributes["src"] = "../../../UserControls/Riesgos/Iframe/IFcategorias.aspx?CodControl=" + codControl +   "&ResponsableEjecucion=" + ResponsableEjecucion + "&IdResponsablesEjecucion=" + IdResponsablesEjecucion +
                "&IdResponsableCalificacion=" + IdResponsableCalificacion + "&Periodicidad=" + Periodicidad + "&test=" + test + "&Reduce=" + Reduce;
            dvIframe.Controls.Add(Iframe);
            trIframe.Visible = true;
        }
            ...
            for (int cellIndex = 0; cellIndex < row.Cells.Count; cellIndex++)
            {
                if (cellIndex == 5)
                {
                    ...
                    if (booActivo == 0)
                    {
                        ...
                        {
                            //continua - inactivar
                            string nuevoEstado = "0";
                            string transaccion = cambiaEstado(nuevoEstado);
                            if (transaccion == "OK")
                            {
                                lbl.Text = "Inactivo";
                                ImgBnt.ToolTip = "Inactivo";
                                ImgBnt.ImageUrl = "~/Imagenes/Icons/switch-off-icon.png";
                                mpeMsgBox1.Hide();
                                omb.ShowMessage("Se inactivó el control satisfactoriamente!", 3, "Atención");
                            }
                        }
                        else
                        {
                            string nuevoEstado = "1";
                            string transaccion = cambiaEstado(nuevoEstado);
                            if (transaccion == "OK")
                            {
                                lbl.Text = "Activo";
                                ...
                                ImgBnt.ImageUrl = "~/Imagenes/Icons/switch-on-icon.png";
                                mpeMsgBox1.Hide();
                                omb.ShowMessage("Se activó el control satisfactoriamente!", 3, "Atención");
                            }
                        }
                    }
                }
        ...
        {//yoendy ca
            ...
            for (int rowIndex = 0; rowIndex < GridView1.Rows.Count; rowIndex++)
            {
                string estado = InfoGridControles.Rows[paginaGrid]["Estado"].ToString().Trim();
