...
{
    public partial class Servicios : System.Web.UI.UserControl
    {
        ...
        protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (GridView1.SelectedRow.RowType == DataControlRowType.DataRow)
            {
                ...
                txtUsuario.Text = GridView1.SelectedDataKey[1].ToString();
                txtFecha.Text = GridView1.SelectedRow.Cells[3].Text;
            }
        }
        ...
        protected void btnImgokEliminar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlDataSource1.DeleteParameters["IdServicio"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtId.Text);
                SqlDataSource1.Delete();
                omb.ShowMessage("La información se eliminó con éxito en la Base de Datos.", 3, "Atención");
            }
            catch (Exception except)
            {
                omb.ShowMessage("Error en la eliminación de la información." + "<br/>" + "Descripción: " + except.Message.ToString(), 1, "Atención");
            }
        }
        ...
        protected void imgBtnInsertar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                txtNombre.Text = "";
                txtUsuario.Text = Session["loginUsuario"].ToString(); // Aca va el Codigo de Usuario logueado en la aplicacion
        ...
        protected void btnImgActualizar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                try
                {
                    SqlDataSource1.UpdateParameters["IdServicio"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtId.Text);
                    SqlDataSource1.UpdateParameters["Descripcion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtNombre.Text);
                ...
                {
                    omb.ShowMessage("Error en la actualización de la información." + "<br/>" + "Descripción: " + except.Message.ToString(), 1, "Atención");
                }
            }
        }
        ...
        protected void btnImgInsertar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            try
            {
                SqlDataSource1.InsertParameters["Descripcion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtNombre.Text);
                SqlDataSource1.InsertParameters["IdUsuario"].DefaultValue = Session["idUsuario"].ToString(); //Aca va el id del Usuario de la BD
                SqlDataSource1.InsertParameters["FechaRegistro"].DefaultValue = System.DateTime.Now.ToString();
            ...
            {
                // Handle the Exception.
                omb.ShowMessage("Error en la inserción de la información." + "<br/>" + "Descripción: " + except.Message.ToString(), 1, "Atención");
            }
        }
        #endregion
        ...
        protected Boolean VerificarCampos()
        {
            ...
            if (ValidarCadenaVacia(Sanitizer.GetSafeHtmlFragment(txtNombre.Text)))
            {
                err = false;
