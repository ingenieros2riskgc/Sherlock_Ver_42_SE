...
namespace ListasSarlaft.UserControls.Parametrizacion
{
    public partial class JerarquiaOrganizacional : System.Web.UI.UserControl
    {
        ...
        private void PopulateTreeViewNewJer()
        {
            DataTable treeViewData = GetTreeViewData();
            AddTopTreeViewNodesNewJer(treeViewData);
        }
        /// <summary>
        ...
        private void AddTopTreeViewNodes(DataTable treeViewData)
        {
            ...
            foreach (DataRowView row in view)
            {
                ...
                //tvNewJerarquia.Nodes.Add(newNode);
                AddChildTreeViewNodes(treeViewData, newNode);
            }
        }
        private void AddTopTreeViewNodesNewJer(DataTable treeViewData)
        {
        ...
        protected void btnImgEditar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            Session["idHijoAnt"] = lblIdHijo.Text.ToString().Trim();
        ...
        protected void imgSelNewJer_Click(object sender, ImageClickEventArgs e)
        {
            pnlDetalle.Visible = false;
            pnlJerarquia.Visible = false;
            btnImgGrabar.Visible = false;
            btnImgActualizar.Visible = false;
            pnlNewJer.Visible = true;
            txtNombreResAnt.Text = txtNombreResponsable.Text;
            txtEmailAnt.Text = txtCorreoResponsable.Text;
            txtCargoResAnt.Text = txtCargoResponsable.Text;
            PopulateTreeViewNewJer();
        }
        ...
        //    try
        //    {
        //        string idHijo = ddlJerarquia.SelectedValue;
        //        DetalleJerarquia(idHijo);
        //    }catch(Exception ex)
        //    {
        //        omb.ShowMessage("Error! Cargando los datos: "+ex.Message, 1, "Atención");
        //    }
        //}
        private void DetalleJerarquia( string idHijo)
        {
            string Detalle = "";
            string selectCommand = "";
            ...
            //    selectCommand = "SELECT NombreResponsable,CorreoResponsable FROM [Parametrizacion].[DetalleJerarquiaOrg] WHERE idHijo = " + idHijo;
            //else
                selectCommand = "SELECT NombreResponsable,CorreoResponsable, NombreHijo,CargoResponsable,JerarquiaOrganizacional.idHijo FROM [Parametrizacion].[JerarquiaOrganizacional] LEFT OUTER JOIN [Parametrizacion].[DetalleJerarquiaOrg] ON [DetalleJerarquiaOrg].idHijo = [JerarquiaOrganizacional].idHijo WHERE [JerarquiaOrganizacional].idHijo = " + idHijo;
            ...
            foreach (DataRowView row in view)
            {
                Session["idHijoNew"] = row["idHijo"].ToString();
                txtNombreResNew.Text =  row["NombreResponsable"].ToString();
                txtEmailNew.Text =  row["CorreoResponsable"].ToString().Trim();
                txtCargoResNew.Text = row["CargoResponsable"].ToString().Trim();
                //if (tipoSelect == 2)
                //    Detalle = Detalle + "\r Nodo Jerarquía Org.: " + row["NombreHijo"].ToString().Trim();
            }
        ...
        private Boolean SaveChangeJer(string idHijo,string idHijoAnt)
        {
            Boolean flag = false;
            string updateCommand = "";

            string conString = WebConfigurationManager.ConnectionStrings["SarlaftConnectionString"].ConnectionString;
            //if(idHijo == string.Empty)
            //{
            //    updateCommand = string.Format("UPDATE [Parametrizacion].[DetalleJerarquiaOrg] SET NombreResponsable='{0}'"+"\n"+
            //        ",CorreoResponsable='{1}',CargoResponsable='{2}' where idHijo = {3}",txtNombreResNew.Text,txtEmailNew.Text,txtCargoResNew.Text,
            //        idHijoAnt);
            //}else
            //{
            //    updateCommand = string.Format("UPDATE [Parametrizacion].[DetalleJerarquiaOrg] SET idHijo ={0}"+"\n"+
            //        "where idHijo={1};" + "\n" +
            //        "UPDATE [Parametrizacion].[JerarquiaOrganizacional] SET idHijo = {0} where idHijo ={1}",idHijo,idHijoAnt);
            //}
            updateCommand = string.Format("UPDATE [Parametrizacion].[JerarquiaOrganizacional] SET idPadre ={0}" + "\n" +
                    "where idHijo={1};"
                    , idHijo, idHijoAnt);
            try
            {
                SqlDataAdapter dad = new SqlDataAdapter(updateCommand, conString);
                DataTable dtblDiscuss = new DataTable();
                dad.Fill(dtblDiscuss);
                flag = true;
            }catch(Exception ex)
        ...
        protected void imgChange_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                string idHijo = Session["idHijoNew"].ToString();
                string idHijoAnt = Session["idHijoAnt"].ToString();
                if(SaveChangeJer(idHijo,idHijoAnt) == true)
                {
                    txtNombreResAnt.Text = string.Empty;
                    txtNombreResNew.Text = string.Empty;
                    txtEmailAnt.Text = string.Empty;
                    txtEmailNew.Text = string.Empty;
                    txtCargoResAnt.Text = string.Empty;
                    txtCargoResNew.Text = string.Empty;
                    pnlDetalle.Visible = false;
                    pnlJerarquia.Visible = true;
                    pnlNewJer.Visible = false;
                    omb.ShowMessage("La Jerarquía ha sido cambiada satisfactoriamente: ", 3, "Atención");
                }
                    
            }
            catch(Exception ex)
            {
                omb.ShowMessage("Error! Guardando los datos: " + ex.Message, 1, "Atención");
            }
        }

        protected void imgCancelNewJer_Click(object sender, ImageClickEventArgs e)
        {
            txtNombreResAnt.Text = string.Empty;
            txtNombreResNew.Text = string.Empty;
            txtEmailAnt.Text = string.Empty;
        ...
        protected void tvNewJerarquia_SelectedNodeChanged(object sender, EventArgs e)
        {
            filaAccion.Visible = true;
            lblIdNuevoHijo.Text = tvNewJerarquia.SelectedNode.Value;

            TreeNode padre = tvNewJerarquia.SelectedNode.Parent;
            if (padre == null)
            {
                lblNodoNuevoPadre.Text = "---";
                lblIdNuevoPadre.Text = "-1";
            }
            else
            {
                lblNodoNuevoPadre.Text = Sanitizer.GetSafeHtmlFragment(padre.Text);
                lblIdNuevoPadre.Text = padre.Value;
            }
            try
            {
                string idHijo = tvNewJerarquia.SelectedNode.Value;
                DetalleJerarquia(idHijo);
            }
            catch (Exception ex)
            {
