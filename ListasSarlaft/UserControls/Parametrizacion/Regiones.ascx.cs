...
{
    public partial class Regiones : System.Web.UI.UserControl
    {
        ...
        protected void btnImgokEliminar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlDataSource1.DeleteParameters["IdRegion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtId.Text.Trim());
        ...
        protected void btnImgActualizar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                if (VerificarCampos())
                {
                    try
                    {
                        SqlDataSource1.UpdateParameters["IdRegion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtId.Text.Trim());
                        SqlDataSource1.UpdateParameters["NombreRegion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtNombre.Text.Trim());
        ...
        protected void btnImgInsertar_Click(object sender, ImageClickEventArgs e)
        {
            if (VerificarCampos())
            {
                ...
                try
                {
                    SqlDataSource1.InsertParameters["NombreRegion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtNombre.Text.Trim());
