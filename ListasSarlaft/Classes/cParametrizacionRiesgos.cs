...
namespace ListasSarlaft.Classes
{
    public class cParametrizacionRiesgos : cPropiedades
    {
        ...
        public DataTable loadInfoClasificacionRiesgo()
        {
            ...
            {
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("SELECT IdClasificacionRiesgo, NombreClasificacionRiesgo, Color FROM Parametrizacion.ClasificacionRiesgo");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public void agregarRegistroTipoPerdida(String NombreTipoPerdidaEvento, int rbtnEstadoActivo)
        {
            try
            {
                cDataBase.conectar();
                cDataBase.ejecutarQuery("INSERT INTO Parametrizacion.TipoPerdidaEvento (NombreTipoPerdidaEvento, Estado) " +
                    "VALUES ('" + NombreTipoPerdidaEvento + "', " + rbtnEstadoActivo + ")");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public void modificarRegistroTipoPerdida(String NombreTipoPerdidaEvento, String IdTipoPerdidaEvento, int estado)
        {
            try
            {
                cDataBase.conectar();
                cDataBase.ejecutarQuery("UPDATE Parametrizacion.TipoPerdidaEvento SET " +
                    "NombreTipoPerdidaEvento = '" + NombreTipoPerdidaEvento + "'," +
                    "Estado = " + estado + " WHERE (IdTipoPerdidaEvento = " + IdTipoPerdidaEvento + ")");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public string consultaestado(string idTipoPerdidaEvento)
        {
            DataTable dtInformacion = new DataTable();
            try
            {
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("SELECT Estado FROM Parametrizacion.TipoPerdidaEvento WHERE IdTipoPerdidaEvento = " + idTipoPerdidaEvento + "");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
            ...
            return dtInformacion.Rows[0]["Estado"].ToString().Trim();
        }
