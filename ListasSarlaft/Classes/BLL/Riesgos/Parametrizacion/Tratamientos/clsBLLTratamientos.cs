...
namespace ListasSarlaft.Classes
{
    public class clsBLLTratamientos
    {
        ...
        public List<clsDTOTratamientos> mtdConsultarTratamientos(ref List<clsDTOTratamientos> lstTratamiento, ref string strErrMsg)
        {
                    ...
                    if (dtInfo != null)
                    {
                        if (dtInfo.Rows.Count > 0)
                        {
                            foreach (DataRow dr in dtInfo.Rows)
                            {
                                ...
                                objTratamiento.strEstado = dr["Estado"].ToString().Trim();
            ...
            {                
                throw new Exception("Error al cargar estados: " + ex.Message.ToString());
            }
        ...
        public int mtdVerificarRelacion(clsDTOTratamientos objTratamiento, ref string strErrMsg)
        {
            int count = 0;
            clsDALTratamientos cDALTratamiento = new clsDALTratamientos();

            count = Convert.ToInt32(cDALTratamiento.mtdUpdateVerificar(objTratamiento, ref strErrMsg));
