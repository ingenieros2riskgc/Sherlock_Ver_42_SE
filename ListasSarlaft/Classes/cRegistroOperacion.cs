    ...
    public class cRegistroOperacion : cPropiedades
    {
        ...
        public DataTable loadRteTraza(string Mes, string Ano,string IdRegistro, string IdEstado)
        {
            ...
            try
            {
                ...
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("SELECT RTRIM(a.Identificacion)Identificacion,RTRIM(a.NombreApellido)NombreApellido,RTRIM(b.NombreTipoRegistro)TipoRegistro,RTRIM(c.NombreEstado)Estado,RTRIM(d.NombreResponsable)UsuarioAsignado,RTRIM(a.NombreIndicador)Indicador,RTRIM(a.Indicador)Descripcion,RTRIM(a.MensajeCorreo)Mensaje,CONVERT(VARCHAR,a.FechaRegistro,120)FechaRegistro,CONVERT(VARCHAR,a.FechaDeteccion,120)FechaDeteccion,CONVERT(VARCHAR,a.FechaPosibleSolucion,120)FechaPosibleSolucion,RTRIM(e.Comentario)Comentario,RTRIM(e.NombreUsuario)NombreUsuario,CONVERT(VARCHAR,e.FechaRegistro,120) FechaRegistroComentario FROM Proceso.RegistroOperacion a LEFT JOIN Proceso.TipoRegistro b on a.IdTipoRegistro = b.IdTipoRegistro LEFT JOIN Proceso.EstadoOperacion c ON a.IdEstadoOperacion = c.IdEstadoOperacion LEFT JOIN Parametrizacion.DetalleJerarquiaOrg d ON a.IdUsuario = d.idHijo LEFT JOIN Proceso.ComentarioRegistroOperacion e ON a.IdRegistroOperacion = e.IdRegistroOperacion WHERE (MONTH(e.FechaRegistro) = '" + Mes + "' AND YEAR(e.FechaRegistro)= '" + Ano + "') OR (MONTH(a.FechaRegistro) = '" + Mes + "' AND YEAR(a.FechaRegistro)= '" + Ano + "') " + cond + " ORDER BY a.Identificacion, e.FechaRegistro");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public DataTable loadInfoRegistroOperacion(String IdTipoRegistro, String IdEstadoOperacion, String IdFactorRiesgo, String IdIndicador, String Identificacion, String FechaDeteccionDesde, String FechaDeteccionHasta, String User,String IdArea, string VerROI)
        {
            ...
            try
            {
                if (IdTipoRegistro != "")
                {
                    condicion = "WHERE (H.IdTipoRegistro = " + IdTipoRegistro + ") ";
                }
                ...
                if (VerROI == "False" )
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE (J.IdEstadoOperacion = 2 ) AND (G.IdUsuario = (select idHijo from Parametrizacion.JerarquiaOrganizacional where idHijo = (select IdJerarquia from listas.Usuarios where IdUsuario = " + IdUsuario.ToString() + " )))";
                    }
                    else
                    {
                        condicion += "AND (J.IdEstadoOperacion = 2 ) AND (G.IdUsuario = (select idHijo from Parametrizacion.JerarquiaOrganizacional where idHijo = (select IdJerarquia from listas.Usuarios where IdUsuario = " + IdUsuario.ToString() + " )))";
                    }
                }
                else
                {
                    if (IdEstadoOperacion != "")
                    {
                        if (condicion.Trim() == "")
                        {
                            condicion = "WHERE (J.IdEstadoOperacion = " + IdEstadoOperacion + ") ";
                        }
                        else
                        {
                            condicion += "AND (J.IdEstadoOperacion = " + IdEstadoOperacion + ") ";
                        }
                    }
                }
                if (IdFactorRiesgo != "")
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE (F.IdFactorRiesgo = " + IdFactorRiesgo + ") ";
                    }
                    else
                    {
                        condicion += "AND (F.IdFactorRiesgo = " + IdFactorRiesgo + ") ";
                    }
                }
                if (IdIndicador != "")
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE (G.IdIndicador = " + IdIndicador + ") ";
                    }
                    else
                    {
                        condicion += "AND (G.IdIndicador = " + IdIndicador + ") ";
                    }
                }
                if (Identificacion != "")
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE (G.Identificacion LIKE N'%" + Identificacion + "%') ";
                    }
                    else
                    {
                        condicion += "AND (G.Identificacion LIKE N'%" + Identificacion + "%') ";
                    }
                }
                if (FechaDeteccionDesde != "")
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE (G.FechaDeteccion >= CONVERT(datetime, '" + FechaDeteccionDesde + "',120)) ";
                    }
                    else
                    {
                        condicion += "AND (G.FechaDeteccion >= CONVERT(datetime, '" + FechaDeteccionDesde + "',120)) ";
                    }
                }
                if (FechaDeteccionHasta != "")
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE (G.FechaDeteccion <= CONVERT(datetime, '" + FechaDeteccionHasta + "',120)) ";
                    }
                    else
                    {
                        condicion += "AND (G.FechaDeteccion <= CONVERT(datetime, '" + FechaDeteccionHasta + "',120)) ";
                    }
                }
                if (IdArea != "" && IdArea != "---")
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE (G.CodigoArea = '" + IdArea + "') ";
                    }
                    else
                    {
                        condicion += "AND (G.CodigoArea = '" + IdArea + "') ";
                    }
                }
                ...
                cDataBase.conectar();
                //09-08-2019
                //dtInformacion = cDataBase.ejecutarConsulta("SELECT	Proceso.RegistroOperacion.IdRegistroOperacion, Proceso.TipoRegistro.NombreTipoRegistro, Proceso.EstadoOperacion.NombreEstado, ISNULL(Parametrizacion.JerarquiaOrganizacional.NombreHijo, '') AS UsuarioAsignado, Proceso.RegistroOperacion.IdUsuario AS idNodoJerarquia, Proceso.RegistroOperacion.Identificacion, Proceso.RegistroOperacion.NombreApellido, Proceso.RegistroOperacion.NombreIndicador, Proceso.RegistroOperacion.Indicador, Proceso.RegistroOperacion.MensajeCorreo, REPLACE(CONVERT(varchar, Proceso.RegistroOperacion.FechaRegistro, 111), '/', '-') AS FechaRegistro, REPLACE(CONVERT(varchar, Proceso.RegistroOperacion.FechaDeteccion, 111), '/', '-') AS FechaDeteccion, ISNULL(REPLACE(CONVERT(varchar, Proceso.RegistroOperacion.FechaPosibleSolucion, 111), '/', '-'), '') AS FechaPosibleSolucion, ISNULL(Proceso.ConteoRegistro.RegistrosCargue, 0) AS RegistrosCargue, ISNULL(Proceso.ConteoRegistro.RegistrosOperacion, 0) AS RegistrosOperacion FROM	Parametrizacion.PerfilSegmento INNER JOIN Parametrizacion.Indicador ON Parametrizacion.PerfilSegmento.IdPerfil = Parametrizacion.Indicador.IdPerfil INNER JOIN Parametrizacion.AtributoSegmentacion ON Parametrizacion.PerfilSegmento.IdAtributo = Parametrizacion.AtributoSegmentacion.IdAtributo AND Parametrizacion.PerfilSegmento.IdAtributo = Parametrizacion.AtributoSegmentacion.IdAtributo AND Parametrizacion.PerfilSegmento.IdAtributo = Parametrizacion.AtributoSegmentacion.IdAtributo AND Parametrizacion.PerfilSegmento.IdAtributo = Parametrizacion.AtributoSegmentacion.IdAtributo INNER JOIN Parametrizacion.TipoSegmento ON Parametrizacion.AtributoSegmentacion.IdTipoSegmento = Parametrizacion.TipoSegmento.IdTipoSegmento INNER JOIN Parametrizacion.Segmento ON Parametrizacion.TipoSegmento.IdSegmento = Parametrizacion.Segmento.IdSegmento AND Parametrizacion.TipoSegmento.IdSegmento = Parametrizacion.Segmento.IdSegmento AND Parametrizacion.TipoSegmento.IdSegmento = Parametrizacion.Segmento.IdSegmento AND Parametrizacion.TipoSegmento.IdSegmento = Parametrizacion.Segmento.IdSegmento AND Parametrizacion.TipoSegmento.IdSegmento = Parametrizacion.Segmento.IdSegmento INNER JOIN F ON Parametrizacion.Segmento.IdFactorRiesgo = Parametrizacion.FactorRiesgo.IdFactorRiesgo AND Parametrizacion.Segmento.IdFactorRiesgo = Parametrizacion.FactorRiesgo.IdFactorRiesgo AND Parametrizacion.Segmento.IdFactorRiesgo = Parametrizacion.FactorRiesgo.IdFactorRiesgo AND Parametrizacion.Segmento.IdFactorRiesgo = Parametrizacion.FactorRiesgo.IdFactorRiesgo AND Parametrizacion.Segmento.IdFactorRiesgo = Parametrizacion.FactorRiesgo.IdFactorRiesgo RIGHT OUTER JOIN Proceso.RegistroOperacion INNER JOIN Proceso.TipoRegistro ON Proceso.RegistroOperacion.IdTipoRegistro = Proceso.TipoRegistro.IdTipoRegistro INNER JOIN Proceso.EstadoOperacion ON Proceso.RegistroOperacion.IdEstadoOperacion = Proceso.EstadoOperacion.IdEstadoOperacion ON Parametrizacion.Indicador.IdIndicador = Proceso.RegistroOperacion.IdIndicador LEFT OUTER JOIN Parametrizacion.JerarquiaOrganizacional ON Proceso.RegistroOperacion.IdUsuario = Parametrizacion.JerarquiaOrganizacional.idHijo LEFT JOIN Proceso.ConteoRegistro ON Proceso.ConteoRegistro.IdConteo = Proceso.RegistroOperacion.IdConteo " + condicion);
                dtInformacion = cDataBase.ejecutarConsulta("SELECT G.IdRegistroOperacion,H.NombreTipoRegistro,J.NombreEstado,ISNULL(K.NombreHijo, '') AS UsuarioAsignado,G.IdUsuario AS idNodoJerarquia,G.Identificacion,G.NombreApellido,G.NombreIndicador,G.Indicador,G.MensajeCorreo,REPLACE(CONVERT(varchar, G.FechaRegistro, 111), '/', '-') AS FechaRegistro,REPLACE(CONVERT(varchar, G.FechaDeteccion, 111), '/', '-') AS FechaDeteccion,ISNULL(REPLACE(CONVERT(varchar, G.FechaPosibleSolucion, 111), '/', '-'), '') AS FechaPosibleSolucion,ISNULL(L.RegistrosCargue, 0) AS RegistrosCargue,ISNULL(L.RegistrosOperacion, 0) AS RegistrosOperacion FROM Parametrizacion.PerfilSegmento AS A WITH(NOLOCK) INNER JOIN Parametrizacion.Indicador AS B WITH(NOLOCK) ON A.IdPerfil = B.IdPerfil INNER JOIN Parametrizacion.AtributoSegmentacion AS C WITH(NOLOCK) ON A.IdAtributo = C.IdAtributo AND A.IdAtributo = C.IdAtributo AND A.IdAtributo = C.IdAtributo AND A.IdAtributo = C.IdAtributo INNER JOIN Parametrizacion.TipoSegmento AS D WITH(NOLOCK) ON C.IdTipoSegmento = D.IdTipoSegmento INNER JOIN Parametrizacion.Segmento AS E WITH(NOLOCK) ON D.IdSegmento = E.IdSegmento AND D.IdSegmento = E.IdSegmento AND D.IdSegmento = E.IdSegmento AND D.IdSegmento = E.IdSegmento AND D.IdSegmento = E.IdSegmento INNER JOIN Parametrizacion.FactorRiesgo AS F WITH(NOLOCK) ON E.IdFactorRiesgo = F.IdFactorRiesgo AND E.IdFactorRiesgo = F.IdFactorRiesgo AND E.IdFactorRiesgo = F.IdFactorRiesgo AND E.IdFactorRiesgo = F.IdFactorRiesgo AND E.IdFactorRiesgo = F.IdFactorRiesgo RIGHT OUTER JOIN Proceso.RegistroOperacion AS G WITH(NOLOCK) INNER JOIN Proceso.TipoRegistro AS H WITH(NOLOCK) ON G.IdTipoRegistro = H.IdTipoRegistro INNER JOIN Proceso.EstadoOperacion AS J WITH(NOLOCK) ON G.IdEstadoOperacion = J.IdEstadoOperacion ON B.IdIndicador = G.IdIndicador LEFT JOIN Listas.Usuarios AS US WITH(NOLOCK) ON G.IdUsuario = US.IdUsuario LEFT OUTER JOIN Parametrizacion.JerarquiaOrganizacional AS K WITH(NOLOCK) ON US.IdJerarquia = K.idHijo LEFT JOIN Proceso.ConteoRegistro AS L WITH(NOLOCK) ON L.IdConteo = G.IdConteo " + condicion);
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
